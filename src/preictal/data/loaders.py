...
def siena_annotations(folder: Path, edfs: list[Path]) -> tuple[dict, list[str]]:
    ...
    for lst in sorted(q for q in folder.rglob("*.txt") if q.name.lower().startswith("seizures-list")):
        ...
        for n, b in enumerate(siena_blocks(lst), 1):
            ...
            if reg is not None and min(abs(reg - t0), 86400 - abs(reg - t0)) > 1:
                notes.append(f"{where}: list gives registration start {reg // 3600:02d}:"
                             f"{reg % 3600 // 60:02d}:{reg % 60:02d} but the EDF header says "
                             f"{t0 // 3600:02d}:{t0 % 3600 // 60:02d}:{t0 % 60:02d}; "
                             "the EDF header time is used")
...
def discover(data_root: str | Path, datasets=tuple(DATASET_PREFIX),
             notes: list[str] | None = None) -> list[Recording]:
    ...
    are appended to `notes` if a list is given.
    """
    ...
    for key in datasets:
        ...
        if key == "chbmit":
            ...
            ann, n = siena_annotations(folder, edfs)
